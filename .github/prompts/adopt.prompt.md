...
5. **Scaffold idempotent.** Copier uniquement les fichiers du template absents du repository cible (README, `.github/copilot-instructions.md` si absent, `.github/PULL_REQUEST_TEMPLATE.md`, `docs/product/plan.md` si absent, `docs/decisions/`, `docs/learnings/`, `docs/independent-verification.md`). Ne jamais écraser un fichier déjà présent (code, README métier, docs existants) sans confirmation explicite du PM. Ensuite, exécuter `specify init` si `.specify/` n'existe pas encore, puis coller le Principe VI (voir `docs/spec-kit-adoption.md` du repo méta) dans `.specify/memory/constitution.md`. Les fichiers de principes d'ingénierie, tests et outillage sont couverts par la constitution spec-kit et ne font plus partie du scaffold.
...
12. **Livraison via PR par défaut.** Pousser la gouvernance ajoutée sur une branche dédiée (ex. `chore/adopt-framework-governance`) et ouvrir une PR avec `.github/PULL_REQUEST_TEMPLATE.md` du scaffold. Si GitHub indique que le repository est mono-contributeur, sans protection de branche et sans PR active, un commit direct sur la branche par défaut est acceptable pour un ajout de gouvernance déterministe. Demander une décision PM seulement si l'information n'est pas déterminable via GitHub, si le repository est partagé/protégé ou si l'action écrase une convention existante.
...
- Ne jamais pousser directement sur la branche par défaut d'un repository ayant des collaborateurs ou une protection de branche sans décision PM explicite.
- Ne pas demander au PM de valider une PR ou un commit d'adoption lorsque les ajouts sont déterministes, non destructifs, vérifiés et sans conflit de gouvernance.
