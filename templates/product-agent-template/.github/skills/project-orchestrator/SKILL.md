...
description: 'Orchestrateur de ce projet. Detecte l etat reel (docs/product/plan.md, GitHub issues, Project, PRs) et route vers le mode approprie : RESUME (reprendre le travail), STATUS (etat sans implementer), REPLAN (faire evoluer le plan), CLOSE (verifier et cloturer). Use when: reprendre ce projet, orchestrer ce projet, ou quand Florent decrit un mode (RESUME/STATUS/REPLAN/CLOSE) sans preciser explicitement quel prompt utiliser.'
...
`florent-product-lab-copilot` (creation de ressources GitHub cross-projets, registre
`projects.yaml`). Ce repository a deja ete bootstrappe.

## Procedure de routage

1. **Si le mode est explicitement donne** par Florent (RESUME/STATUS/REPLAN/CLOSE) :
   verifier sa coherence avec l'etat detecte avant d'executer (voir tableau ci-dessous).
...
2. **Si aucun mode n'est donne** : deduire le mode le plus probable a partir de l'etat
   detecte (issues ouvertes, PRs en cours, ecart plan/realite), puis le proposer a
   Florent avant de l'executer — ne jamais executer un mode devine sans confirmation.
